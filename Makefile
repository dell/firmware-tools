...
pkglibdir = $(libdir)/firmware-tools
pkgincludedir = $(includedir)/firmware-tools
...
DIST_COMMON = README $(am__configure_deps) $(dist_sbin_SCRIPTS) \
	$(pkgpython_PYTHON) $(srcdir)/Makefile-std \
...
am__installdirs = "$(DESTDIR)$(bindir)" "$(DESTDIR)$(sbindir)" \
	"$(DESTDIR)$(pkgpythondir)" "$(DESTDIR)$(pkgdatadir)" \
	"$(DESTDIR)$(pkgetcdir)"
...
nodist_pkgetcDATA_INSTALL = $(INSTALL_DATA)
DATA = $(nodist_pkgdata_DATA) $(nodist_pkgetc_DATA)
...
ACLOCAL = ${SHELL} /home/mdomsch/git/firmware-tools/pkg/missing --run aclocal-1.10
AMTAR = ${SHELL} /home/mdomsch/git/firmware-tools/pkg/missing --run tar
AUTOCONF = ${SHELL} /home/mdomsch/git/firmware-tools/pkg/missing --run autoconf
AUTOHEADER = ${SHELL} /home/mdomsch/git/firmware-tools/pkg/missing --run autoheader
AUTOMAKE = ${SHELL} /home/mdomsch/git/firmware-tools/pkg/missing --run automake-1.10
AWK = gawk
CYGPATH_W = echo
DEFS = -DPACKAGE_NAME=\"firmware-tools\" -DPACKAGE_TARNAME=\"firmware-tools\" -DPACKAGE_BUGREPORT=\"\" -DPACKAGE=\"firmware-tools\" -DVERSION=\"1.5.11\" -DPACKAGE_VERSION=\"1.5.11\" -DPACKAGE_STRING=\"firmware-tools\ 1.5.11\"
ECHO_C = 
ECHO_N = -n
ECHO_T = 
...
LTLIBOBJS = 
MAINT = #
MAKEINFO = ${SHELL} /home/mdomsch/git/firmware-tools/pkg/missing --run makeinfo
MKDIR_P = /bin/mkdir -p
PACKAGE = $(RELEASE_NAME)
PACKAGE_BUGREPORT = 
PACKAGE_NAME = firmware-tools
PACKAGE_STRING = firmware-tools 1.5.11
...
abs_builddir = /home/mdomsch/git/firmware-tools
abs_srcdir = /home/mdomsch/git/firmware-tools
abs_top_builddir = /home/mdomsch/git/firmware-tools
abs_top_srcdir = /home/mdomsch/git/firmware-tools
am__leading_dot = .
...
install_sh = $(SHELL) /home/mdomsch/git/firmware-tools/pkg/install-sh
...
pdfdir = ${docdir}
pkgpyexecdir = ${pyexecdir}/firmware-tools
pkgpythondir = ${pythondir}/firmware-tools
prefix = /usr/local
...
install-nodist_pkgetcDATA: $(nodist_pkgetc_DATA)
	@$(NORMAL_INSTALL)
	test -z "$(pkgetcdir)" || $(MKDIR_P) "$(DESTDIR)$(pkgetcdir)"
	@list='$(nodist_pkgetc_DATA)'; for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  f=$(am__strip_dir) \
	  echo " $(nodist_pkgetcDATA_INSTALL) '$$d$$p' '$(DESTDIR)$(pkgetcdir)/$$f'"; \
	  $(nodist_pkgetcDATA_INSTALL) "$$d$$p" "$(DESTDIR)$(pkgetcdir)/$$f"; \
	done

uninstall-nodist_pkgetcDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(nodist_pkgetc_DATA)'; for p in $$list; do \
	  f=$(am__strip_dir) \
	  echo " rm -f '$(DESTDIR)$(pkgetcdir)/$$f'"; \
	  rm -f "$(DESTDIR)$(pkgetcdir)/$$f"; \
	done
...
installdirs-am:
	for dir in "$(DESTDIR)$(bindir)" "$(DESTDIR)$(sbindir)" "$(DESTDIR)$(pkgpythondir)" "$(DESTDIR)$(pkgdatadir)" "$(DESTDIR)$(pkgetcdir)"; do \
...
install-data-am: install-nodist_pkgdataDATA install-nodist_pkgetcDATA \
...
uninstall-am: uninstall-binPROGRAMS uninstall-dist_sbinSCRIPTS \
	uninstall-nodist_pkgdataDATA uninstall-nodist_pkgetcDATA \
	uninstall-pkgpythonPYTHON
